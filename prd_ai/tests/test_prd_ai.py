...
class TestPRDContext(common.TransactionCase):
    """6.1: prd_id på session + resolver prd_partner."""
    ...
    def setUpClass(cls):
        ...
        cls.coworker = cls.env['ai.coworker'].create({
            'name': 'Test PRD',
            'status': 'active',
            'cost_context_partner_strategy': 'prd_partner',
        })
    ...
    def test_capture_sets_prd_id(self):
        """6.1a: _capture_context(prd=...) sätter prd_id."""
        sess = self._session()
        sess._capture_context(prd=self.prd)
        self.assertEqual(sess.prd_id, self.prd)
    ...
    def test_resolver_prd_partner_author(self):
        """6.1c: resolver härleder partner från författare."""
        sess = self._session(prd_id=self.prd.id)
