...
class PrdRequirement(models.Model):
    ...
    def _compute_function_names(self):
        for record in self:
            ...
            for func in record.function_ids:
                ...
                state = (
                    dict(func._fields["state"].selection).get(func.state, "")
                    if func.state
                    else ""
                )
                if name and state:
    ...
    category = fields.Many2many(
        comodel_name="prd.requirement_category", string="Tags", help="Categories"
    )
    code = fields.Char(
        string="Code", size=8, trim=True, help="Requirement Number, unique ID"
    )
    description = fields.Text(string="Description")
    duration_tracking = fields.Float(string="Duration Tracking")
    function_ids = fields.One2many(
        comodel_name="prd.requirement.function", inverse_name="req_id"
    )
    function_names = fields.Char(string="Functions", compute="_compute_function_names")
    function_names_ids = fields.Many2many(
        comodel_name="prd.function",
        string="Function",
        compute="_compute_function_names_ids",
    )
    ...
    object_id = fields.Reference(
        string="Object",
        selection=lambda m: [
            (model.model, model.name) for model in m.env["ir.model"].sudo().search([])
        ],
        help="Requirement from this object",
    )
    parent_id = fields.Many2one(
        comodel_name="prd.requirement", compute="_compute_parent_id", store=True
    )
    partner_id = fields.Many2one(
        comodel_name="res.partner", string="Stake Holder", help=""
    )
    ...
    req_type = fields.Many2one(
        comodel_name="prd.requirement_type",
        string="Type",
        help="",
        domain="['|',('prd_id','=',prd_id),('prd_id','=',prd_parent_id)]",
    )
    ...
    def get_system_report_data(self, req_ids):
        ...
        for req in req_ids:
            for func in [
                f.func_id
                for f in req.function_ids
                if f.func_type.implementation_type == "module"
            ]:
                data[func.name] = {
                    "name": func.name,
                    "description": func.description or "",
                    "requirements": ",".join(
                        [r.code for r in func.requirement_ids.mapped("req_id")]
                    ),
                    "complexity": dict(func._fields["weight"].selection).get(
                        func.weight, ""
                    ),
...
class PRDRequirementFunction(models.Model):
    ...
    func_type = fields.Many2one(
        comodel_name="prd.function_type", string="Type", related="func_id.func_type"
    )
    req_id = fields.Many2one(
        comodel_name="prd.requirement", string="", help="", ondelete="cascade"
    )
    req_state = fields.Selection(related="req_id.state", string="State")
    req_type = fields.Many2one(
        comodel_name="prd.requirement_type", string="Type", related="req_id.req_type"
    )
    sequence = fields.Integer(string="Sequence")
    state = fields.Selection(
        [("draft", "Draft"), ("ongoing", "Ongoing"), ("done", "Done")],
...
class RequirementType(models.Model):
    ...
    def _total(self):
        ...
        if active_model == "prd.requirement":
            ...
            for req in self.env["prd.requirement"].browse(active_ids):
                tot[req.req_type.id] += sum(
                    [int(t.weight) for t in req.function_ids.mapped("func_id")]
                )
            for tid in tot.keys():
