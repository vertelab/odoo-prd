    ...
    def get_contents(self, repository, path, branch):
        ...
        try:
            ...
            try:
                ...
                tree = repository.repository_tree(path=path, ref=branch, recursive=False, get_all=True)
    ...
    def normalize_file_object(self, file_object):
        ...
        try:
            ...
            if hasattr(file_object, 'file_path'):
                ...
                relative_path = '/'.join(path_parts[1:]) if len(path_parts) > 1 else file_object.file_path
                
                # Construct GitLab raw file URL
                # Format: https://git.vertel.se/project/path/-/raw/branch/file/path
                download_url = ''
                if hasattr(file_object, '_project_web_url') and hasattr(file_object, '_branch'):
